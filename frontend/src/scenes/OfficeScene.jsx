...
export default function OfficeScene() {
  ...
  useFrame(() => {
    ...
    const distance = new THREE.Vector3(...userPosition).distanceTo(seatingArea)
  ...
  return (
    <>
      ...
      {[-18, -12, -6, 0, 6, 12, 18].map((x, i) => (
        <mesh key={`plank-${i}`} rotation={[-Math.PI / 2, 0, 0]} position={[x, 0.01, 0]} receiveShadow>
      ...
      {[-6, -3, 0, 3, 6].map((x, i) => (
        [-6, -3, 0, 3, 6].map((z, j) => (
          <group key={`light-${i}-${j}`} position={[x, 5.9, z]}>
            ...
            <pointLight position={[0, -0.1, 0]} intensity={0.4} color="#ffffff" />
          </group>
        ))
      ))}
